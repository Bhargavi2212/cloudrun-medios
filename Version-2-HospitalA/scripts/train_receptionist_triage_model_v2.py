...
def tune_logistic_regression(
    ...
    logger.info(
        f"Running grid search on sampled data (10 combinations, "
        f"{cv}-fold CV, parallelized)..."
    )
    start_time = time.time()
...
def main():
    ...
    for feature_set in feature_sets:
        ...
        if feature_set_results:
            ...
            logger.info(
                f"ESI 1-2 Recall: "
                f"{feature_set_results[best_model_name]['metrics']['esi_1_2_recall']:.4f}"
            )
    ...
    if test_results:
        ...
        report_lines.append(
            f"- **Recommended Model**: {best_result['model']} with "
