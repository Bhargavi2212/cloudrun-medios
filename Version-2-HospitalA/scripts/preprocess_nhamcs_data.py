...
def cap_outliers(
    ...
    for feature, (lower, upper) in bounds.items():
        ...
        logger.info(
            f"  Capped (lower): train={train_lower}, "
            f"val={val_lower}, test={test_lower}"
        )
        logger.info(
            f"  Capped (upper): train={train_upper}, "
            f"val={val_upper}, test={test_upper}"
        )
...
def apply_yeo_johnson(
    ...
    for feature in features_to_transform:
        ...
        logger.info(
            f"  {transformed_feature}: {skew:.3f} " f"(improvement: {improvement:.1f}%)"
        )
...
def assemble_final_features(
    ...
    logger.info(
        f"  X_train_final: {X_train_final.shape[0]:,} rows x "
        f"{X_train_final.shape[1]} columns"
    )
    logger.info(
        f"  X_val_final: {X_val_final.shape[0]:,} rows x "
        f"{X_val_final.shape[1]} columns"
    )
    logger.info(
        f"  X_test_final: {X_test_final.shape[0]:,} rows x "
        f"{X_test_final.shape[1]} columns"
    )
...
def detect_data_leakage(
    ...
    logger.info(f"\nSaved leakage check to: {OUTPUT_DIR / 'data_leakage_check.txt'}")
...
def test_transformer_loading() -> None:
    ...
    logger.info(
        f"\nSaved test results to: {OUTPUT_DIR / 'transformer_loading_test.txt'}"
    )
...
def generate_preprocessing_report(
    ...
    with open(OUTPUT_DIR / "preprocessing_report.md", "w") as f:
        f.write(report_text)

    logger.info(
        f"Saved comprehensive report to: {OUTPUT_DIR / 'preprocessing_report.md'}"
    )
