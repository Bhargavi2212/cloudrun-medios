"""
Combine NHAMCS ED data files (2011-2022) into a unified dataset for
triage model training.
...
def clean_and_transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "TEMPF" in df.columns:
        ...
        print(
            f"  TEMPF stats (raw integer): min={df['TEMPF'].min():.1f}, "
            f"max={df['TEMPF'].max():.1f}, "
            f"non-null={df['TEMPF'].notna().sum():,}"
        )
        ...
        print(
            f"  temp_c stats: min={df['temp_c'].min():.1f}, "
            f"max={df['temp_c'].max():.1f}, "
            f"non-null={df['temp_c'].notna().sum():,}"
        )
    ...
    if "esi_level" in df.columns:
        ...
        # Actually, pandas handles float(1.0) == int(1) in isin(),
        # so we can keep as float
...
def main() -> None:
    ...
    print(
        f"SUCCESS: Clean dataset: {len(df_clean):,} records "
        f"({len(df_clean)/len(df_combined)*100:.1f}% retained)"
    )
    ...
    print(
        f"SUCCESS: Dataset saved: {len(df_clean):,} records, "
