...
3. Set environment variables:
   ...
   - `MANAGE_AGENT_STORAGE_ROOT` - Storage directory (default: ./storage)
...
#### Upload Document
```bash
curl -X POST "http://localhost:8001/manage/documents/upload" \
  -F "file=@test.pdf" \
...
```bash
curl "http://localhost:8001/manage/documents/pending-review"
```

#### Process Document
```bash
curl -X POST "http://localhost:8003/summarizer/documents/<file-id>/process"
```

#### Confirm Document
```bash
curl -X POST "http://localhost:8001/manage/documents/<file-id>/confirm" \
  -F "notes=Approved by nurse"
```

#### Reject Document
```bash
curl -X POST "http://localhost:8001/manage/documents/<file-id>/reject" \
