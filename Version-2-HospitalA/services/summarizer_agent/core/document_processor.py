...
class DocumentProcessor:
    ...
    def __init__(
        ...
        if genai is None:
            logger.warning(
                "google-generativeai not installed, document processing "
                "will be disabled"
            )
            return
    ...
    async def _step1_clean_text(
        ...
        system_instruction = (
            "You are a medical document text cleaning expert. Clean "
            "OCR/PDF text, fix errors, identify sections, and normalize "
            "formatting. Return only the cleaned text, no JSON."
        )
...
Return only valid JSON, no markdown formatting."""
    ...
    async def _step3_deidentify(
        ...
        system_instruction = (
            "You are a PHI de-identification expert. Remove all personally "
            "identifiable information from medical data. Return valid JSON "
            "only."
        )
...
Return the same JSON structure with PHI replaced."""
    ...
    async def _step4_score_confidence(
        ...
        system_instruction = (
            "You are a medical data quality expert. Score confidence "
            "(0-100%) for extracted fields. Return valid JSON only."
        )
