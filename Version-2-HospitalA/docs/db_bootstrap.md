...
- **Service container**
  ...
  - Password: `postgres`
  - Databases:
    - `medi_os_v2_a` (created by the container)
    - `medi_os_v2_a_test` (created in the workflow)

The CI workflow (`.github/workflows/ci.yml`) sets:

```text
DATABASE_URL=postgresql+asyncpg://postgres:postgres@localhost:5432/medi_os_v2_a
TEST_DATABASE_URL=postgresql+asyncpg://postgres:postgres@localhost:5432/medi_os_v2_a_test
```
