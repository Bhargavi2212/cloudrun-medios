...
Write-Host '  $env:DATABASE_URL="postgresql+asyncpg://user:pass@localhost:5432/dbname"' -ForegroundColor Yellow
Write-Host ""
Write-Host "To start services, run in separate terminals:" -ForegroundColor Cyan
Write-Host "  Terminal 1: uvicorn services.manage_agent.main:app --port 8001 --reload" -ForegroundColor White
Write-Host "  Terminal 2: uvicorn services.summarizer_agent.main:app --port 8003 --reload" -ForegroundColor White
