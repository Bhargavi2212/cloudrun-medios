...
from sqlalchemy import (JSON, BigInteger, CheckConstraint, Column, Date,
                        DateTime)
...
class AccessToken(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        Index("ix_access_tokens_user_purpose", "user_id", "purpose", "consumed_at"),
    )
...
class Consultation(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    status: Mapped[ConsultationStatus] = mapped_column(
        SAEnum(ConsultationStatus), nullable=False
    )
    ...
    assigned_doctor: Mapped[Optional[User]] = relationship(
        "User", backref="consultations"
    )
    timeline_events: Mapped[List["TimelineEvent"]] = relationship(
        "TimelineEvent",
...
class Vital(TimestampMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        Index("ix_vitals_consultation_time", "consultation_id", "recorded_at"),
    )
...
class LabResult(TimestampMixin, SoftDeleteMixin, Base):
    ...
    consultation: Mapped[Optional[Consultation]] = relationship(
        "Consultation", backref="lab_results"
    )
    __table_args__ = (
        Index("ix_lab_results_consultation", "consultation_id", "collected_at"),
    )
...
class QueueState(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    current_stage: Mapped[QueueStage] = mapped_column(
        SAEnum(QueueStage), nullable=False
    )
...
class QueueEvent(TimestampMixin, Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    queue_state_id: Mapped[str] = mapped_column(
        ForeignKey("queue_states.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        Index("ix_queue_events_state_time", "queue_state_id", "created_at"),
    )
...
class AudioFile(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    consultation: Mapped[Optional[Consultation]] = relationship(
        "Consultation", backref="audio_files"
    )

    __table_args__ = (
        Index("ix_audio_files_consultation", "consultation_id", "created_at"),
    )
...
class Transcription(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    audio_file_id: Mapped[str] = mapped_column(
        ForeignKey("audio_files.id", ondelete="CASCADE"), unique=True, nullable=False
    )
...
class TriagePrediction(TimestampMixin, SoftDeleteMixin, Base):
    ...
    consultation: Mapped[Consultation] = relationship(
        "Consultation", backref="triage_predictions"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "esi_level BETWEEN 1 AND 5", name="ck_triage_predictions_esi_range"
        ),
    )
...
class PatientSummary(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    summary_type: Mapped[SummaryType] = mapped_column(
        SAEnum(SummaryType), nullable=False
    )
    content: Mapped[Optional[str]] = mapped_column(Text())
    ...
    __table_args__ = (
        Index("ix_patient_summaries_patient", "patient_id", "created_at"),
    )
...
class SummaryCache(Base):
    ...
    summary_id: Mapped[Optional[str]] = mapped_column(
        ForeignKey("patient_summaries.id", ondelete="SET NULL")
    )
