...
class ManageAgentWaitTimePredictor:
    ...
    def _ml_based_prediction(self, patient: PatientQueueItem, queue_patients: List[PatientQueueItem]) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_names = [
                'urgency_level', 'medical_complexity', 'queue_length', 'staff_available', 'predicted_triage_level',
                'hour_of_day', 'day_of_week', 'is_weekend', 'is_rush_hour', 'is_night',
                'hour_sin', 'hour_cos', 'day_sin', 'day_cos', 'triage_complexity_interaction',
                'triage_queue_interaction', 'triage_staff_interaction', 'triage_queue_staff_interaction',
                'high_priority', 'low_priority', 'emergency_case', 'critical_case', 'patient_staff_ratio',
                'staff_efficiency', 'load_factor', 'staff_utilization', 'queue_length_squared',
                'queue_length_cubed', 'medical_complexity_squared', 'triage_level_squared',
                'staff_available_squared', 'complexity_queue_staff_interaction', 'department_encoded'
            ]
