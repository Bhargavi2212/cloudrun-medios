...
[Treatment plan to be documented]
    ...
    async def process_audio_pipeline(
        ...
        *,
        ...
        try:
            ...
            logger.info(f"📊 Recorded LLM usage: model={model_name}, tokens={result.get('tokens_prompt', 0)}+{result.get('tokens_completion', 0)}, status={'success' if is_success else 'failed'}, has_note={has_note}")
        ...
        logger.info(f"🔍 Note saving check: consultation_id={consultation_id}, has_generated_note={bool(result.get('generated_note'))}, generated_note_length={len(result.get('generated_note', ''))}")
        ...
        if consultation_id and result.get("generated_note"):
            ...
            if not critical_errors:
                try:
                    ...
                    if note_content:  # Only save if we have actual content
                        ...
                        logger.info(f"✅ Note saved successfully to consultation {consultation_id} (is_stub={result.get('is_stub', False)}, content_length={len(note_content)})")
                    else:
                    ...
                    logger.error(f"❌ Failed to save note to consultation {consultation_id}: {exc}", exc_info=True)
                    result["note_saved"] = False
            ...
            logger.warning(f"⚠️ No consultation_id provided - note will not be saved. Result has generated_note: {bool(result.get('generated_note'))}")
            result["note_saved"] = False
    ...
    def _persist_note(
        ...
        *,
        ...
        try:
            logger.info(f"💾 Starting note persistence: consultation_id={consultation_id}, content_length={len(note_content)}, is_stub={is_stub}")
            ...
            with get_session() as session:
                ...
                logger.info(f"✅ Note persisted successfully: note_id={version.note_id}, version_id={version.id}, consultation_id={consultation_id}")
                return version
        except Exception as exc:
            logger.error(f"❌ Error persisting note for consultation {consultation_id}: {exc}", exc_info=True)
            raise
