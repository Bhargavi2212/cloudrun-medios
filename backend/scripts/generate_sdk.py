...
def generate_sdk_with_openapi_generator(
    ...
    cmd = [
        ...
        "generate",
        "-i", str(openapi_schema_path.absolute()),
        "-g", language,
        "-o", str(output_dir.absolute()),
        "--package-name", package_name,
        "--package-version", package_version,
        "--additional-properties", f"npmName={package_name},npmVersion={package_version}" if language in ("typescript", "javascript") else "",
    ]
    ...
    if language not in ("typescript", "javascript"):
        ...
        for i, arg in enumerate(cmd):
            if skip_next:
                skip_next = False
                continue
            if arg == "--additional-properties" and language not in ("typescript", "javascript"):
                skip_next = True
                continue
...
def generate_sdk_with_docker(
    ...
    docker_cmd = [
        "docker", "run", "--rm",
        "-v", f"{schema_abs.parent}:/local/input",
        "-v", f"{output_abs.parent}:/local/output",
        "openapitools/openapi-generator-cli", "generate",
        "-i", f"/local/input/{schema_abs.name}",
        "-g", language,
        "-o", f"/local/output/{output_abs.name}",
        "--package-name", package_name,
    ]
...
def generate_typescript_sdk_simple(openapi_schema_path: Path, output_dir: Path) -> bool:
    ...
    try:
        ...
        cmd = [
            ...
            "--input", str(openapi_schema_path.absolute()),
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="typescript",
        choices=["typescript", "python", "javascript", "go", "java", "php", "ruby", "swift", "kotlin"],
