...
rfv1_count = df[rfv_cols[0]].notna().sum()
rfv1_pct = rfv1_count / len(df) * 100
print(f"Records with RFV1: {rfv1_count:,} ({rfv1_pct:.2f}%)")
if len(rfv_cols) > 1:
    rfv2_count = df[rfv_cols[1]].notna().sum()
    rfv2_pct = rfv2_count / len(df) * 100
    print(f"Records with RFV2: {rfv2_count:,} ({rfv2_pct:.2f}%)")
if len(rfv_cols) > 2:
    rfv3_count = df[rfv_cols[2]].notna().sum()
    rfv3_pct = rfv3_count / len(df) * 100
    print(f"Records with RFV3: {rfv3_count:,} ({rfv3_pct:.2f}%)")
