...
def load_and_modify_params(model_name: str, feature_set: str) -> dict:
    """Load best hyperparameters from v2 and replace class_weight."""
...
def train_logistic_regression_no_tuning(
    ...
    """Train Logistic Regression using best params from v2, in batches."""
    logger.info("\n" + "=" * 80)
...
def train_decision_tree_no_tuning(
    ...
    """Train Decision Tree using best params from v2, in batches."""
    logger.info("\n" + "=" * 80)
...
def train_random_forest_no_tuning(
    ...
    """Train Random Forest using best params from v2, in batches."""
    logger.info("\n" + "=" * 80)
...
def train_xgboost_no_tuning(
    ...
    """Train XGBoost using best params from v2, in batches."""
    logger.info("\n" + "=" * 80)
        ...
        for model_name, train_func in models_config.items():
            try:
                ...
                params_file = (
                    OUTPUT_DIR
                    / "parameters"
                    / f"used_params_{model_name_safe}_{feature_set}.json"
                )
                ...
                for dataset_name, metrics in eval_results.items():
                    metrics_file = (
                        OUTPUT_DIR
                        / "metrics"
                        / f"{dataset_name}_metrics_{model_name_safe}_{feature_set}.csv"
                    )
...
def generate_comparison_report(all_results: dict, output_dir: Path) -> None:
    ...
    report_lines.append("")
    report_lines.append(
        "| Model | Feature Set | Accuracy | ESI 1-2 Recall | "
        "ESI 3-4 Recall | Macro F1 | Train-Val Gap |"
    )
    report_lines.append(
        "|-------|-------------|----------|----------------|----------------|----------|---------------|"
    )
