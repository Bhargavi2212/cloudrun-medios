...
def train_logistic_regression_no_tuning(
    ...
    """Train Logistic Regression using best params from v2."""
    logger.info("\n" + "=" * 80)
...
def train_stacking_ensemble(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(
        ...
        logger.info(
            f"  Fold {fold_idx}/{cv}: Training base models on "
        ...
        try:
            ...
            for dataset_name, metrics in eval_results.items():
                ...
                metrics_file = (
                    OUTPUT_DIR
                    / "metrics"
                    / f"{dataset_name}_metrics_{model_name_safe}.csv"
                )
...
def generate_comparison_report(all_results: dict, output_dir: Path) -> None:
    ...
    report_lines.append(
        "- **SMOTE Strategy**: ESI 1->5000, 2->15000, "
    ...
    report_lines.append("")
    report_lines.append(
        "| Model | Accuracy | ESI 1-2 Recall | ESI 3-4 Recall | "
    ...
    if best_overall:
        ...
        if esi_1_2_recall >= 0.40:
            ...
            report_lines.append(
                f"- [WARN] ESI 1-2 recall ({esi_1_2_recall:.2%}) "
                "is below target (>= 40%)"
            )
        ...
        if esi_3_4_recall >= 0.50:
            ...
            report_lines.append(
                f"- [WARN] ESI 3-4 recall ({esi_3_4_recall:.2%}) "
                "is below target (>= 50%)"
            )
