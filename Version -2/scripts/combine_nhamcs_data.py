"""
Combine NHAMCS ED data files (2011-2022) into unified dataset for triage training.
...
def clean_and_transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "esi_level" in df.columns:
        ...
        # pandas handles float(1.0) == int(1) in isin(), so we can keep as float
...
def main() -> None:
    ...
    for year in years:
        ...
        if year == 2012 and not data_file.exists():
            ...
            if data_file.exists():
                ...
                # For .sav, we'd need pyreadstat, skip for now
                # or use existing combined CSV
                continue
    ...
    retained_pct = len(df_clean) / len(df_combined) * 100
    print(
