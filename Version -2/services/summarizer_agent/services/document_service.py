    ...
    async def process_document(self, file_id: UUID) -> dict[str, Any]:
        ...
        if asset.content_type == "application/pdf":
            ...
            if extracted_text:
                ...
                # Use existing raw_text if available
                raw_text = asset.raw_text
            else:
                logger.warning(
                    "PDF extraction failed and no raw_text available for file %s",
                    file_id,
                )
                raise ValueError(
            ...
            if not raw_text:
                logger.warning(
                    "No raw_text available for file %s (content_type: %s)",
                    file_id,
                    asset.content_type,
                )
                raise ValueError(
                    f"No text available for document type: {asset.content_type}"
                )
