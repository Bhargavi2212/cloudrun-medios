...
| POST | `/manage/federation/train` | Launch a local federated training round and submit model weights. |
...
| `ORCHESTRATOR_BASE_URL` | Orchestrator URL used for registry and patient snapshots. |
| `ORCHESTRATOR_SHARED_SECRET` | Shared secret for orchestrator requests. |
| `FEDERATION_BASE_URL` | Aggregator URL for submitting model updates. |
| `FEDERATION_SHARED_SECRET` | Shared secret used with the aggregator. |
| `TRIAGE_FEATURES_PATH` / `TRIAGE_LABELS_PATH` | Paths to the processed training datasets. |
