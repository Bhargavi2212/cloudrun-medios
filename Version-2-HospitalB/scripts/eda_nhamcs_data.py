...
def outlier_analysis(df: pd.DataFrame) -> None:
    ...
    for var in numerical_vars:
        ...
        if outlier_count > 0:
            print(
                f"  Min Outlier: {outliers.min():.2f}, Max Outlier: {outliers.max():.2f}"  # noqa: E501
            )
...
def bivariate_analysis_cat_cat(df: pd.DataFrame) -> None:
    ...
    for var in categorical_vars:
        ...
        print(
            f"{var}: Chi2={chi2:.2f}, p={p_value:.4f}, significant={'Yes' if p_value < 0.05 else 'No'}"  # noqa: E501
        )
...
def bivariate_analysis_num_cat(df: pd.DataFrame) -> None:
    ...
    for num_var in numerical_vars:
        ...
        print(
            f"{num_var}: F={f_stat:.2f}, p={p_value:.4f}, significant={'Yes' if p_value < 0.05 else 'No'}"  # noqa: E501
        )
